...
class TestBaseStorage(TestCase):
    ...
    def test_edit_batch_unknown_type(self):
        ...
        self.assertEqual([BatchEditResult(edits[0], False, 'unsupported_edit_type', 'Mutation')], results)
    ...
    def test_edit_batch_exception(self):
        ...
        edits = [Delete('foobar')]
        results = edit_batch(storage, edits)
        self.assertEqual([BatchEditResult(edits[0], False, 'exception', 'i_dont_like_you')], results)

    def test_search_no_limit(self):
        storage = MemStorage(get_storage_meta(SuperBowlResult), {n['code']: n for n in SUPER_BOWL_RESULT_DICTS})
        result_set = storage.search(FieldFilter('year', FieldFilterOp.lte, 1967))
        expected = ResultSet(results=[{
            'code': 'i',
            'year': 1967,
