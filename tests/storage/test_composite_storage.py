...
class TestCompositeStorage(TestCase):
    ...
    def create_composite_storage():
        ...
        storage = CompositeStorage(
            storage=tuple(mem_storage(dataclasses.replace(storage_meta, name=n))
                          for n in ('a', 'b')),
        )
    ...
    def test_search_all_with_search_filter(self):
        ...
        expected = {('a', 'b')[i % 2]+'/'+str(i): f'Item {i}' for i in range(95, 100)}
    ...
    def test_search_with_search_filter_on_key_and_sort_order(self):
        ...
        expected = [dict(id=('a', 'b')[i % 2] + '/' + str(i), title=f"Item {i}") for i in range(99, 94, -1)]
        self.assertEqual(expected, results)
