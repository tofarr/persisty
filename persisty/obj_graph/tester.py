...
from persisty.obj_graph.resolver.resolver_descriptor import ResolverDescriptor
...
class MemberEntity(EntityABC, Member):
    ...
    band: 'BandEntity' = ResolverDescriptor(BelongsTo(f'{__name__}.BandEntity', 'band_id'))


class BandEntity(EntityABC, Band):
    __persisty_context__ = persisty_context
    members: MemberEntity = ResolverDescriptor(HasMany(f'{__name__}.MemberEntity',
                                               search_filter_factory=lambda b: MemberFilter(band_id__eq=b.id),
                                               inverse_attr='_band'))
