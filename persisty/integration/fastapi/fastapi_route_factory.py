...
class FastApiRouteFactory:
    ...
    def get_storage(self, authorization: Authorization) -> StorageABC:
        return self.persisty_context.get_storage(
    ...
    def create_route_for_count(self, api: FastAPI):
        ...
        @api.get(
            f"/storage/{self.storage_meta.name}/search", response_model=int
        )
        async def count(
            search_filter: Optional[search_filter_model] = Depends(search_filter_model),
            authorization: Authorization = Depends(self.get_authorization),
    ...
    def create_route_for_update(self, api: FastAPI):
        ...
        @api.patch(
            self.get_key_path(), response_model=Optional[item_model]
        )
        async def update(
            key: str = Path(),
    ...
    def create_route_for_edit_batch(self, api: FastAPI):
        ...
        @api.post(f"/storage/{self.storage_meta.name}/batch", response_model=List[Optional[item_model]])
        async def edit_batch(
            edits: List[edit_model] = Body(), authorization: Authorization = Depends(self.get_authorization)
        ) -> int:
