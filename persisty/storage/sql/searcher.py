...
class Searcher(Generic[T]):
    ...
    def search(self, cursor, storage_filter: Optional[StorageFilter[T]] = None, batch_size: int = 100) -> Iterator[T]:
        where = self._build_select_where(storage_filter)
        try:
    ...
    def paged_search(self, cursor, storage_filter: Optional[StorageFilter[T]] = None, page_key: Optional[str] = None,
                     limit: int = 20):
        where = self._build_select_where(storage_filter)
    ...
    def _build_select_where(self, storage_filter: Optional[StorageFilter] = None):
        where = build_where(storage_filter.item_filter if storage_filter else None, self._sql_table)
        sql = self._sql
        if where.sql:
            sql = f'{sql} WHERE {where.sql}'
        order = self._build_order(storage_filter.item_comparator if storage_filter else None)
        if order:
