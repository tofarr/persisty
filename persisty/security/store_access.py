...
class StoreAccess:
    ...
    def editable(self):
        return (
            self.create_filter != EXCLUDE_ALL or self.update_filter != EXCLUDE_ALL or self.delete_filter != EXCLUDE_ALL
        )
    ...
    def item_updatable(self, item, updates, attrs: Tuple[Attr, ...]):
        ...
        if not self.update_filter.match(item, attrs):
            return False
        new_item = dataclasses.replace(item, **{k: v for k, v in dataclasses.asdict(updates) if v is not UNDEFINED})
