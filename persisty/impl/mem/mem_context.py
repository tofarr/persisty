...
from persisty.access_control.factory.access_control_factory import (
    READ_ONLY_ACCESS_FACTORY,
)
...
from persisty.context.obj_timestamp import (
    TimestampStorageABC,
    TimestampUpdateStorage,
    timestamp_storage,
)
...
class MemContext(PersistyContextABC):
    ...
    def get_storage(
        ...
        storage_meta = dataclasses.replace(
            storage.storage_meta, access_control=access_control
        )
    ...
    def get_meta_storage(self, authorization: Authorization) -> MetaStorageABC:
        access_control = self.meta_access_control_factory.create_access_control(
            "meta", authorization
        )
    ...
    def get_timestamp_storage(
        ...
        storage_meta = dataclasses.replace(
            self.timestamp_storage.storage_meta, access_control=access_control
        )
