...
from aaaa.impl.sqlalchemy.search_filter.search_filter_converter_context import (
    SearchFilterConverterContext,
)
from aaaa.impl.sqlalchemy.sqlalchemy_column_converter import POSTGRES
from aaaa.search_filter.exclude_all import EXCLUDE_ALL
from aaaa.store.batch_edit import BatchEdit
from aaaa.store.batch_edit_result import BatchEditResult
from aaaa.store.result_set import ResultSet
...
from aaaa.util import UNDEFINED, from_base64, to_base64
...
class SqlalchemyTableStore(StoreABC):
    ...
    def _load(self, item: Dict):
        ...
        for attr_ in self.store_meta.attrs:
            ...
            if (
                attr_.type == AttrType.JSON
                and not self.engine.dialect.name == POSTGRES
            ):
                value = json.loads(value)
            elif attr_.type == AttrType.DATETIME and value:
                if not value.tzinfo:
    ...
    def _dump(self, item: ExternalItemType, is_update: bool):
        ...
        for attr_ in self.store_meta.attrs:
            ...
            if attr_.type == AttrType.DATETIME and value:
