...
class UserFilteredStorage(FilteredStorage[T, F, S]):
    """ Performance of this may be degraded if the search_filter does not have the user id field"""
    ...
    def filter_filter(self, search_filter: Optional[F]) -> Tuple[Optional[F], bool]:
        ...
        if not hasattr(search_filter_type, filter_field_name):
            return search_filter, False  # The search search_filter does not properly handle the constraint
