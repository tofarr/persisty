# Persisty Example App : messenger : Part 5

In [the previous step](../messenger_5), using SQL as our persistence mechanism.
...
Appsync requires that api keys be used to access your API, so we added an action to [auth.py](messenger/actions/auth.py)
...
We now have a version of the messenger app deployed to AWS. A further step could be to use
